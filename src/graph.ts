/**
 * The work graph — the "deploy many agents" half of the design.
 *
 ...
 *
 * Each iteration of the loop deploys a small fleet: one observer agent per
 * check (all concurrent), a diagnoser, a proposer, three critic agents with
 * fresh contexts (concurrent), a gate, and a single serial verify tail.
 * Every node declares an output contract; a result that does not match its
 * shape is rejected and retried rather than passed downstream, so a
 * malformed result fails where it was produced instead of corrupting
